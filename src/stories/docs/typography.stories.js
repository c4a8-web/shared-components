...
const getTypeFaceLevel = (typeFace) => {
  ...
  return typeFaceSegments[0] === "font"
    ? "font-size-" + typeFaceSegments[2]
    : typeFaceSegments[0];
};
...
const generateDocs = function (typeFace, breakpoints) {
  ...
  let docs = "";
  let aboveBreakpointLabel =
    breakpoints.length > 1 ? "Above (lg) Breakpoint" : "All Breakpoints";
  ...
  breakpoints.forEach(function (breakpoint) {
    ...
    if (typeFaceValue.indexOf("calc(") !== -1) {
      const pxValueWithRem = typeFaceValue
        .replace("calc(", "")
        .replace(")", "")
        .replace("rem", "");
